...
### よく使う操作例
- **`ysiw"`** : カーソル下の単語を `"` で囲む
- **`ysi)"`** : カッコ `()` の中身を `"` で囲む ( `func(arg)` -> `func("arg")` )
- **`cs'"`** : `'text'` を `"text"` に変更する
- **`ds"`** : `"text"` を `text` にする (囲みを消す)
- **`v` (選択) + `S"`** : 選択した範囲を `"` で囲む

---

## 標準操作：テキストオブジェクト (`ci"`, `di(`, etc.)

**`操作` + `i (inner)` + `対象`** の順で入力することで、囲みの中身だけを操作できます。

| 操作 | 対象 | 説明 |
| :--- | :--- | :--- |
| **`ci"`** | `"` | `""` の中身を書き換える (Change Inside) |
| **`di"`** | `"` | `""` の中身を削除する (Delete Inside) |
| **`vi"`** | `"` | `""` の中身だけを選択する (Visual Inside) |
| **`yi"`** | `"` | `""` の中身だけをコピーする (Yank Inside) |
| **`ci(`** | `(` | `()` の中身を書き換える |
| **`ci{`** | `{` | `{}` の中身を書き換える |
| **`cit`** | `t` | HTML/XMLタグ (`<div>...</div>`) の中身を書き換える |
